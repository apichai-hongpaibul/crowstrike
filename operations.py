    ...
    def get_devices_ran_on(self, hash_type, hash_value, count_only):
        ...
        if count_only:
            ...
            try:
                ...
                return {'device_count': resources[0].get('device_count', 0) if resources else 0}
        ...
        result = {}
        result.update({'device_count': len(device_response['resources'])})
        result.update({'device_ids': device_response['resources']})
        return result
...
def scan_host(config, params):
    obj = Falcon_RTR(config)
    endpoint = '/ods/entities/scans/v1'
    cpu = int(params.get("cPU"))
    if cpu not in range(1,5):
        raise ConnectorError(f"Invalid CPU.")
    host_id = params.get("hostId")
    file_path = params.get("filePath")
        
    payload = {
      "cloud_ml_level_detection": 0,
      "cloud_ml_level-prevention": 0,
      "cpu_priority": cpu,
      "description": "fortisoar scan",
...
def get_scan_result(config, params):
    obj = Falcon_RTR(config)
    endpoint = '/ods/entities/scans/v1'
...
def query_device(config, params):
    obj = Falcon_RTR(config)
    endpoint = '/devices/queries/devices/v1'
    source_ip = params.get("sourceIP")
    payload = {
      "filter": "local_ip:'{0}'".format(source_ip),
      "limit": scan_id,
      "offset": scan_id
    }
    logger.debug('payload=> {0}'.format(payload))
...
def check_online_state(config, params):
    obj = Falcon_RTR(config)
    endpoint = '/devices/entities/online-state/v1'
...
def get_device_entities(config, params):
    obj = Falcon_RTR(config)
    endpoint = '/devices/entities/devices/v2'
    device_id = params.get("deviceId")
    payload = {
...
operations = {
    ...
    'check_online_state': check_online_state,
    'get_device_entities': get_device_entities
}
